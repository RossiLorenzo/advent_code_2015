# --- Day 16: Aunt Sue ---
#   
# Your Aunt Sue has given you a wonderful gift, and you'd 
# like to send her a thank you card. However, there's a small 
...
# Analysis Machine! Just what you wanted. Or needed, as the case 
# may be.
# 
# The My First Crime Scene Analysis Machine (MFCSAM for short) 
# can detect a few specific compounds in a given sample, as well 
# as how many distinct kinds of those compounds there are. According 
...
# It beeps inquisitively at you a few times and then prints out a 
...
input = readLines("~/Google Drive/Lorenzo/AdventCalendar/input16.txt")
to_extract = c("children", "cats", "samoyeds", "pomeranians",
               "akitas", "vizslas", "goldfish", "trees",
               "cars", "perfumes")
input_df = data_frame(
  index = as.numeric(gsub(":.*", "", gsub("Sue ", "", input)))
) %>% 
  cbind(lapply(to_extract, function(x){
    as.numeric(gsub(",.*", "", gsub(paste0(".*", x,": "), "", input)))
  }) %>% 
    do.call(cbind, .))
names(input_df)[-1] = to_extract
...
# MFCSAM's instructions catches your eye. 
...
# the pomeranians and goldfish readings indicate that there 
# are fewer than that many (due to the modial interaction of 
# magnetoreluctance).
# 
