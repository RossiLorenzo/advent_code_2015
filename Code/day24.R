...
# of a group of packages is the product of their weights, that is, the value you get when you 
# multiply their weights together. Only consider quantum entanglement if the first group has the 
...
# For this situation, the unique first groups, their quantum entanglements, and a way to divide 
...
# Read Input
input = as.numeric(readLines("../Inputs/input24.txt"))

# Function to find all good comibinations
balance_sleigh = function(input, n_groups){
  # Start from 2 items and go up until don't find some groups
  total_sum = sum(input)
  i = 2
  found_combination1 = NULL
  while(length(found_combination1) == 0 & i < length(input)){
    print(paste("Trying with groups of size", i))
    x = combinations(length(input), i, input, set = FALSE)
    all_sums = apply(x, 1, sum)
    found_combination1 = which(all_sums == total_sum / n_groups)
    i = i + 1
  }
  x[found_combination1, ]
}

# Solution part1
group1 = balance_sleigh(input, 3)
min(apply(group1, 1, prod))
