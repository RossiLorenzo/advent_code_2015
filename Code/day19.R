...
# Rudolph is going to need custom-made medicine. Unfortunately, Red-Nosed Reindeer chemistry 
...
# capable of constructing any Red-Nosed Reindeer molecule you need. It works by starting with 
# some input molecule and then doing a series of replacements, one per step, until it has the right 
# molecule.
# 
...
input = readLines("~/Google Drive/Lorenzo/AdventCalendar/input19.txt")
starting_molecule = input[length(input)]
input = input[1:(length(input) - 2)]
input = data_frame(From = gsub(" .*", "", input), To = gsub(".* ", "", input))

# For every subsitution get where is the match and what could be the replacement
all_res = unique(unlist(lapply(unique(input$From), function(i){
  tmp = filter(input, From == i)
  l = stri_locate_all(starting_molecule, fixed = i)[[1]]
  if(is.na(l[1,1]))
    return(NULL)
  new_strings = unique(unlist(lapply(tmp$To, function(j){
    apply(l, 1, function(x){
    x = as.numeric(x)
    # Add before the substitution
    new_text = ifelse(x[1] > 1, 
                      substr(starting_molecule, 1, x[1] - 1),
                      "")
    # Add the substituion
    new_text = paste0(new_text, j)
    # Add the end of the string
    if(x[2] < nchar(starting_molecule))
      new_text = paste0(new_text, substr(starting_molecule, x[2] + 1, nchar(starting_molecule)))
    return(new_text)
  })})))
})))
