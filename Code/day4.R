...
# hash starting with five zeroes is 1048970; that is, 
...
i = 1
while(!found){
  found = all(strsplit(digest(paste0(input, i), "md5", serialize = FALSE), "", fixed = TRUE)[[1]][1:5] == 0)
  i = i + 1
}
paste("The number is", i - 1, "and the hash is", digest(paste0(input, i - 1), "md5", serialize = FALSE))
...
found = FALSE
while(!found){
  found = all(strsplit(digest(paste0(input, i), "md5", serialize = FALSE), "", fixed = TRUE)[[1]][1:6] == 0)
  i = i + 1
}
paste("The number is", i - 1, "and the hash is", digest(paste0(input, i - 1), "md5", serialize = FALSE))
