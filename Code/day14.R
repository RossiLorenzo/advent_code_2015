...
# Read input and reshape
input = readLines("~/Google Drive/Lorenzo/AdventCalendar/input14.txt")
input_df = data_frame(
  Name = gsub(" .*", "", input),
  Speed = as.numeric(gsub(" .*", "", gsub(".*fly ", "", input))),
  Stamina = as.numeric(gsub(".* ", "", gsub(" seconds,.*", "", input))),
  Rest = as.numeric(gsub(" .*", "", gsub(".*for ", "", input)))
)

# Every (stamina + rest) seconds the deer does stamina * speed kms.
deer_distance = function(input_df, time_available){
  input_df %>%
    group_by(Name) %>% 
    mutate(PeriodTime = Stamina + Rest,
           PeriodDistance = Stamina * Speed,
           PeriodsAvailable = time_available / PeriodTime,
           FullPeriods = floor(PeriodsAvailable),
           LeftTime = time_available - PeriodTime * FullPeriods,
           Distance = PeriodDistance * FullPeriods + min(c(Stamina, LeftTime)) * Speed) %>% 
    ungroup()
}
deer_distance(input_df, 2503)
...
# for the lead, they each get one point.) He keeps the traditional 2503 second time 
...
# Dancer had been in the lead for the 139 seconds before that, he has accumulated 
...
deer_distance_points = function(input_df, time_available){
  tmp = input_df %>%
    group_by(Name) %>% 
    mutate(PeriodTime = Stamina + Rest,
           PeriodDistance = Stamina * Speed,
           ...
           Distance = PeriodDistance * FullPeriods + min(c(Stamina, LeftTime)) * Speed) %>% 
    ungroup()
  tmp$Name[tmp$Distance == max(tmp$Distance)]
}
result = sapply(1:2503, function(i){ deer_distance_points(input_df, i) })
table(unlist(result))
