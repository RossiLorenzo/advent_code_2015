...
# cookie recipe. All you have to do is find the right balance 
# of ingredients.
# 
...
# finish the recipe (your puzzle input) and their properties 
# per teaspoon:
#   
...
# and you have to be accurate so you can reproduce your results 
# in the future. The total score of a cookie can be found by adding 
# up each of the properties (negative totals become 0) 
...
# it would have instead become zero, causing the whole score to 
# multiply to zero.
# 
...
input = readLines("~/Google Drive/Lorenzo/AdventCalendar/input15.txt")
numbers = do.call(rbind, Map(as.numeric, str_extract_all(input, "(-|)\\d+")))
colnames(numbers) = c("capacity", "durability", "flavor", "texture", "calories")
input_df = data_frame(name = gsub(":.*", "", input)) %>% 
  cbind(numbers)

# The composition score will be a matrix * vector multiplication
# I cna ismulate every combinations like this
cores = makeCluster(10)
res = rbind_all(parLapplyLB(cores, 0:100, function(i, numbers){
  library(dplyr)
  rbind_all(lapply(0:(100 - i), function(j){
    rbind_all(lapply(0:(100 - i - j), function(h){
      k = 100 - i - j - h
      cross_product = t(numbers) %*% c(i,j,h,k)
      cross_product[cross_product < 0] = 0
      data_frame(i, j, h, k, score = prod(cross_product[1:4]), calories = cross_product[5,1])
    }))
  }))
}, numbers))
stopCluster(cores)
...
# make another recipe that has exactly 500 calories per cookie 
# (so they can use it as a meal replacement). Keep the rest of your 
...
# 40*8 + 60*3 = 500. The total score would go down, though: 
