...
// [[Rcpp::export]]
long int find_position(int row_pos, int col_pos, long int starting_number) {
  int row_n = 1;
  int max_row_n = 1;
  int col_n = 1;
  while(row_n != row_pos | col_n != col_pos){
    // If I reached the top go down
    if(row_n == 1){
      row_n = max_row_n + 1;
      col_n = 1;
      max_row_n++;
    ...
    starting_number = (starting_number * 252533) % 33554393;
  }
...
# ago, and we just finished shredding all of the manuals. I bet we can find you the code generation 
# algorithm, though."
# 
...
# filled in diagonally up and to the right. This process repeats until the infinite paper is covered. 
...
# 
# Santa looks nervous. Your puzzle input contains the message on the machine's console. What code 
# do you give the machine?

# Libraries
library(stringi)

# Read Input
input = readLines("../Inputs/input25.txt")
input = as.numeric(stri_extract_all(input, regex = "[0-9]+")[[1]])

# Part 1 solution
find_position(input[1], input[2], 20151125)
*/
