...
# (forward or backward, respectively). For example, jmp +1 would simply continue with the 
# next instruction, while jmp +0 would continuously jump back to itself forever.
# 
...
# INPUT
input = readLines("~/Google Drive/Lorenzo/AdventCalendar/input23.txt")
input = strsplit(gsub(",|\\+", "", input), " ", fixed = TRUE)

# Function to execute operations
simulate_steps = function(a, b){
  i = 1
  while(i <= length(input)){
    ops = input[[i]]
    reg = ops[2]
    if(ops[1] == "hlf")
      assign(reg, get(reg)/2)
    if(ops[1] == "tpl")
      assign(reg, get(reg) * 3)
    if(ops[1] == "inc")
      assign(reg, get(reg) + 1)
    if(ops[1] == "jmp")
      i = i + as.numeric(ops[2]) - 1
    if(ops[1] == "jie")
      if(get(reg) %% 2 == 0)
        i = i + as.numeric(ops[3]) - 1
    if(ops[1] == "jio")
      if(get(reg) == 1)
        i = i + as.numeric(ops[3]) - 1
    i = i + 1
